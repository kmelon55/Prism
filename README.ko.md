...
Prism 0.1.1은 **고정된 로컬 인증서로 서명되었으며 Apple 공증은 받지 않았습니다.**
...
새 패키지 빌드는 고정된 서명 인증서를 사용합니다. 기존 임시 서명 설치본에서 전환할 때는
권한을 한 번 다시 허용해야 할 수 있습니다. 이후 서명이 호환되지 않는 업데이트는 앱을 교체하기 전에 차단합니다.
업데이트 파일 서명과 Apple 공증은 별개이며, [서명 설정](docs/releases.md#signing-and-notarization)을 참고하세요.
