...
@MainActor final class PrismModifierShortcuts {
    ...
    private var recordingPrimary: UInt32?
    ...
    func blockRecordingModifiers(_ kinds: Set<UInt32>, allowingPrimary: UInt32? = nil) {
        if !blocked.isEmpty { ignoreUntil = ProcessInfo.processInfo.systemUptime + 0.42 }
        blocked = kinds; recordingPrimary = allowingPrimary; reset()
    }
    ...
    func handle(_ event: NSEvent) {
        ...
        for kind in enabled.sorted() {
            if blocked.contains(kind), kind != recordingPrimary { detectors[Int(kind)].reset(); continue }
