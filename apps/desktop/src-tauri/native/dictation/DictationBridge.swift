...
final class DictationController: ObservableObject {
    ...
    var busy: Bool { ["preparing", "recording", "transcribing", "inserting"].contains(phase) }
    func emit(action: String? = nil) {
        ...
        var value: [String: Any] = ["phase": phase, "message": message, "microphone": permission,
            "accessibility": AXIsProcessTrusted(), "hasTranscript": !lastTranscript.isEmpty, "session": generation, "shortcutWarning": controls.failedIDs.isEmpty ? "" : language.text("녹음 단축키를 등록하지 못했습니다. 다른 앱의 단축키 또는 손쉬운 사용 권한을 확인하세요.", "A recording shortcut could not be registered. Check other apps’ shortcuts or Accessibility permission.")]
    ...
    func stop(pressEnter: Bool = false, paste: Bool? = nil) {
        ...
        work = Task {
            ...
            do {
                ...
                if config.provider == "local" {
                    ...
                    raw = try await RemoteTranscriptionService().transcribe(audioURL: audio,
                ...
                let text = TextPostProcessor.clean(raw, vocabulary: config.vocabulary)
    ...
    private func installCancel() {
        ...
        controls.onCopy = { [weak self] in self?.stop(paste: false) }
        controls.onPaste = { [weak self] in self?.stop(paste: true) }
        controls.onSend = { [weak self] in self?.stop(pressEnter: true, paste: true) }
