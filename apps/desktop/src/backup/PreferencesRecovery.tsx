...
export function PreferencesRecovery({ review, onRecover, onReviewAgain }: {
  ...
  return <section className="backup-review" aria-label={t("Recover local settings")}>
    ...
    {review ? <><AnimatedDetails><summary>{t("Review last-known-good settings")}</summary><dl>
    ...
    </dl></AnimatedDetails><button disabled={busy || failed} onClick={() => void recover()}>{t("Recover last-known-good settings")}</button></> : <p>{t("No valid settings snapshot is available. You can review a backup file to restore safe preferences.")}</p>}
