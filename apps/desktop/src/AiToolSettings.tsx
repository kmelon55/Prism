...
export function AiToolSettings({ nativeRuntime }: { nativeRuntime: boolean }) {
  ...
  return <section className="ai-tool-settings" aria-label={t("AI 도구와 접근 권한")}>
    ...
    <label className="ai-output-limit"><span><strong>{t("출력·추론 한도")}</strong><small>{t("추론 모델이 답변 전에 한도를 소진하면 높여 주세요. 한도가 클수록 사용량이 늘 수 있습니다.")}</small></span><select aria-label={t("출력 토큰 한도")} disabled={disabled} value={value.maxOutputTokens} onChange={e=>void update(()=>setAiTools({...value,maxOutputTokens:Number(e.target.value)}))}><option value={4096}>{t("4,096 토큰")}</option><option value={16384}>{t("16,384 토큰")}</option><option value={32768}>{t("32,768 토큰")}</option></select></label>
