...
Ordinary reads now use a scoped `SecKeychainSetUserInteractionAllowed(false)`
...
Allow key use action. Successful authorization is reused by both chat and
dictation; renderer responses contain only the masked key and status.
...
Cmd+Q has no Quit action. Explicit Quit and updater restart retain Tauri's normal
